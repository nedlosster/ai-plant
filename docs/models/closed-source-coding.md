...
**SWE-bench Verified**: K2.5 76.8%
**SWE-Bench Pro**: **K2.6 58.6%** (лидер open-weight, апрель 2026)
**Контекст**: 128K

**Kimi K2.6** (20-22 апреля 2026) -- новое поколение с четырьмя вариантами (Instant, Thinking, Agent, **Agent Swarm**). Agent Swarm -- native multi-agent orchestration из коробки. HLE 54.0% (SOTA), **SWE-bench Verified 80.2%** (близко к Opus 4.6 80.8%), SWE-Bench Pro **58.6%** (выше GLM-5.1 58.4%), SWE-bench Multilingual 76.7%, **Quality Index 53.9** (live open-source лидер). Open-source под Apache 2.0. Для нашей платформы -- по-прежнему API-only (не помещается).
...
| **Kimi K2.6** (open Apache 2.0) | **80.2%** | **58.6%** | -- | TBA | TBA | 128K | нет (240+ GB) | Agent Swarm native |
