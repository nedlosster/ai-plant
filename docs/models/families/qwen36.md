...
- **Anthropic API protocol** -- работает с Claude Code и совместимыми клиентами
...
- **Сложные workflow** в agent-инструментах (Claude Code, opencode, Aider, Cline)
...
- **opencode / Claude Code / Aider / Cline** -- бэкенд для agent-style кодинга
...
# Совместимые клиенты:
# - Claude Code (через Anthropic API protocol)
# - opencode (через OpenAI-compatible endpoint)
# - Qwen Code, Kilo Code, Cline, OpenClaw
```
