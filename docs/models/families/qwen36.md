...
| tg на платформе | **58.7 tok/s** (замер) | ~15 tok/s (оценка) | +3.9× |
...
| **Qwen3.6-27B (dense, лидер SWE-V)** | 23 апр 2026 | 27B dense | **17 GiB** | 🔴 **рекомендуется скачать** |
...
1. **Сейчас** (когда current full Coder Next прогон закончится):
   - Скачать **Qwen3.6-27B Q4_K_M** (~17 GiB) -- лидер open-weight SWE-V
   - Создать `qwen3.6-27b.sh` preset (с `-c 131072`, `--keep 1500`, --batch-size/--ubatch-size 4096)
   - Запустить smoke + --tries 2 (~2 ч) для baseline -- ожидаем 70-78% pass_rate_2
