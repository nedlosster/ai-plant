...
| 2 | [Qwen3-Coder Next 80B-A3B](families/qwen3-coder.md#next-80b-a3b) | 70.6% | 53 | Лучший agent-style на скорости, 256K контекст |
| 3 | [Devstral 2 24B](families/devstral.md) | 72.2% | ~25 | Лидер dense-сегмента 24B, FIM+agent в одной модели |
| 4 | [Qwen3-Coder 30B-A3B](families/qwen3-coder.md#30b-a3b) | ~62% | **86** | Самый быстрый chat по коду, идеален для коротких запросов |
| 5 | [Qwen2.5-Coder 32B](families/qwen25-coder.md#32b) | -- | ~12 | HumanEval 92.7%, эталон FIM, 32B dense |
| 6 | [Codestral 25.08](families/codestral.md) | -- | ~28 | Лидер LMSYS Copilot Arena по FIM, 80+ языков |
...
**#2 Qwen3-Coder Next** -- основной выбор для daily agentic-кодинга. 80B MoE с 3B активных параметров: качество как у 24B dense, скорость почти как у 3B. 256K контекст уверенно держит средний monorepo. Проигрывает Qwen 3.6-27B и Devstral 2 на сложных SWE-задачах, но выигрывает по скорости генерации.

**#3 Devstral 2 24B** -- лидер dense-семейства 24B с 72.2% SWE-V. Tool use дисциплина строже MoE-вариантов, итерации более последовательные. Минус -- 25 tok/s ощутимо медленнее MoE на той же платформе.

**#4 Qwen3-Coder 30B-A3B** -- младшая MoE того же семейства. 86 tok/s даёт ощущение мгновенного ответа, идеален для "объясни функцию", "перепиши этот блок", быстрых code review. Слабее Coder Next на multi-file задачах из-за меньшего числа экспертов.

**#5 Qwen2.5-Coder 32B** -- классика dense-сегмента. HumanEval 92.7% всё ещё впечатляет, FIM-токены работают эталонно. Минус -- 12 tok/s и контекст 128K делают его непрактичным для agent loop, но как FIM-сервер премиум-уровня жив.

**#6 Codestral 25.08** -- лидер FIM в LMSYS Copilot Arena. 80+ языков покрытия, 256K контекст. Лицензия MNPL (не Apache), что для коммерческих сценариев требует внимания.
...
[Qwen 3.6-27B](families/qwen36.md#27b) -- максимум качества SWE-V локально (**77.2%**, лидер open-source local). Выбор когда важно качество multi-step reasoning над скоростью. Multimodal -- скриншоты UI прямо в agent loop. ~17 GiB, ~15 tok/s (оценка).
[Qwen3-Coder Next](families/qwen3-coder.md#next-80b-a3b) -- лидер MoE efficiency (70.6% [SWE-bench Verified](../llm-guide/benchmarks/swe-bench.md) при 3B active), 256K контекст, 53 tok/s. Выбор для daily-loop когда нужна скорость. Практические workflow'ы -- [coding/workflows.md](../coding/workflows.md#workflow-1-fim--cli-agent-основной).
...
| Скриншот ошибки → код | [Qwen 3.6-27B](families/qwen36.md#27b) или [Gemma 4 26B-A4B](families/gemma4.md) + mmproj | 8084 / 8081 | vision из коробки |
