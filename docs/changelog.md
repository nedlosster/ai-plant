...
- Создана папка `docs/ai-agents/agents/claude-code/` с 6 статьями (2460 строк):
  - `README.md` -- профиль + index
  - `news.md` -- хроника фич
  - `agent-teams.md` -- стратегия Agent Teams (3 паттерна, Code Kit v5.0 YAML, playbooks)
  - `skills-guide.md` -- как писать Skills, 7 best practices
  - `hooks-guide.md` -- Hooks для safety (git guardrails, secret scanning, audit log)
  - `mcp-setup.md` -- MCP-серверы (30+ популярных, Python/TypeScript SDK)
...
- Создана `docs/models/families/glm.md` (208 строк) -- полная карточка семейства Z.ai:
...
- Обновлены `closed-source-coding.md`, `coding.md`, `vision.md`
...
- `docs/models/news.md`: три новые записи
...
- Создана `docs/models/closed-source-coding.md` (289 строк) -- обзор GPT-5.3 Codex, Claude Opus/Sonnet/Mythos, Gemini 3.1 Pro, Kimi K2.5. Decision matrix API vs local, pricing per-task
- Расширен `docs/models/families/gemma4.md` (+170 строк): варианты E2B/E4B/26B/31B, adaptive image tokens, PLE, bounding box prediction, 4 новых сценария
- Refresh news: MCP 97M установок (де-факто стандарт), Gemma 4 перенесена из Q4 2025 → Q2 2026 (fix даты)
- Коммиты: `2fbc0ad`, `53d4463`

## 2026-04-11

**Большая сессия: apps/, TTS integration, review, inference profiles**

Самый активный день -- 7 коммитов. Основные события:

- **Новый раздел `docs/apps/`** (4873 строки в 21 файле): профили end-user приложений
  - `comfyui/`, `open-webui/`, `lobe-chat/`, `ace-step/` -- каждый с README + introduction + architecture + simple/advanced use-cases
- **TTS integration** (733 строки): `open-webui/tts-integration.md` + `lobe-chat/tts-integration.md` -- Kokoro, Chatterbox, XTTS, Whisper setup
...
- **Inference profiles** (1722 строки):
  - `llama-cpp.md` (585) -- история, GGML, GGUF, квантизации, бэкенды
  - `ollama.md` (606) -- docker-стиль, OCI-registry, content-addressed storage
  - `lemonade.md` (531) -- NPU-ускорение для Ryzen AI, hybrid prefill/decode
- **Refresh news Apr 2026**: Cursor 3, Devin 2.0, SWE-bench 93.9%, Claude Code 41% market share
- **Хроника релизов моделей**: `docs/models/news.md` создан (142 строки)
...
- **Бенчмарки** (`llm-guide/benchmarks/`):
  - `HumanEval.md`, `SWE-bench.md` перенесены в benchmarks/
  - Новые: `LiveCodeBench.md`, `MMMU.md`
  - `README.md` -- классификация бенчмарков, рейтинговые сайты
- **Сравнение моделей платформы с frontier closed-source** (`models/README.md` -- таблицы бенчмарков)
- **Hardware**:
  - `enterprise-inference.md` -- datacenter GPU, API pricing, self-hosted vs cloud
  - `hardware-alternatives.md` -- RTX 5090, Mac M4 Max, DGX Spark, M3 Ultra
  - `acceleration-outlook.md` -- перспективы GPU/CPU/NPU
- **ROCm фиксы**:
...
- **Статья про плотность знаний в моделях** (`llm-guide/model-information-density.md`) -- парадокс сжатия, биты на параметр, MoE
...
- **Gemma 4** (`models/families/gemma4.md`): оптимальная стратегия, 7 простых + 8 сложных кейсов, антипаттерны
- **InternVL** (`models/families/internvl.md`): InternVL3.5 38B, простые/сложные кейсы
- **Qwen3-VL**:
...
- **Coding стратегия** (`models/coding.md`): топ open-моделей, strategy opencode, open vs cloud, слухи Q2-Q3 2026
- **Qwen3.5**: раздел про агенты кодинга
- **Kimi K2.5** добавлена в families
- **AI-agents раздел**:
  - README.md навигация
  - Markdown-ссылки на agents/ из models/
  - Удалён `use-cases/coding/opencode/` (консолидировано в agents/opencode.md)
  - `commercial.md`, `open-source.md` как индексы
- **Function calling** -- раздел про платформу, колонка FC в таблицах, поле в шапках
...
- **Inference-пресеты** (10 коммитов):
  ...
  - Рефакторинг пресетов в `scripts/inference/{vulkan,rocm}/preset/`
...
- **Gemma 4 vision через `--mmproj`**: создана статья `docs/models/vision.md`
- **TTS**: статья про voice cloning + `scripts/tts/` для TTS-WebUI
- **Docs**: расширены `vision.md`, `llm.md`, `coding.md`, `video.md` актуализацией 2026
...
- `docs/platform/processor.md`: макс память 128 GiB (8ch x 2rank x 16Gbit), бенчмарки
...
- Создан раздел `docs/ai-agents/` -- обзор индустрии AI-агентов для разработки
- Коммиты: `f1bd373`
...
- `docs/llm-guide/rag.md` (монолит) разделён на 7 статей в `docs/llm-guide/rag/`:
...
- Базовая структура `docs/`: platform, inference, models, llm-guide, training, use-cases
- Скрипты: `scripts/inference/`, `scripts/webui/`, `scripts/music/`, `scripts/tts/`
...
| 2026-04-11 | **Самый активный день**: раздел `docs/apps/`, inference profiles, глоссарий, refresh news |
...
- **Разделов `docs/`**: 8 (platform, inference, models, llm-guide, training, ai-agents, use-cases, apps)
