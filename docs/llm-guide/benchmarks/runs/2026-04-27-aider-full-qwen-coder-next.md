...
### Per-language breakdown (агрегация из .aider.results.json)

Финальный YAML агрегат записан после 178 задач без per-language footer. Ниже -- агрегация per-task `.aider.results.json` файлов через [`scripts/inference/aider-aggregate.sh`](../../../../scripts/inference/aider-aggregate.sh) (179 задач: финальный YAML 178 + 1 cpp задача из resume #14, успевшая зачислиться перед diamond loop):

| Язык | Прогнано / в датасете | Pass 1-try | Pass 2-tries | Rate 1 | Rate 2 |
|------|----------------------|------------|--------------|--------|--------|
| **JavaScript** | 47/49 (96%) | 21 | **38** | 44.7% | **80.9%** ⭐⭐ |
| **Python** | 34/34 (100%) ✅ | 11 | **26** | 32.4% | **76.5%** ⭐ |
| **C++** | 18/26 (69%) | 6 | 11 | 33.3% | 61.1% |
| **Java** | 44/47 (94%) | 11 | 26 | 25.0% | 59.1% |
| **Go** | 36/39 (92%) | 11 | 21 | 30.6% | 58.3% |
| **Итого** | **179/195** (91.8%) | **60** | **122** | **33.5%** | **68.2%** |

**Retry effect: +34.6pp** -- лидер платформы.

Наблюдения:

- **JavaScript -- абсолютный лидер на Coder Next**: 80.9% pass_rate_2, в **2.5× выше** чем у 30B-A3B (32.7% на 49 задачах). 80B размер критически помогает на JS-задачах с error feedback от Jasmine/Jest
- **Python -- стабильно высокий** (76.5%) с **100% покрытием** -- все 34 задачи пройдены без watchdog зависаний. Самый "понятный" язык для модели
- **C++ -- weak spot покрытия** (69%): 8 из 26 задач недоступны из-за reasoning loops (diamond, zebra-puzzle и подобные). Качество на пройденных нормальное (61%)
- **Java и Go -- средне** (~58-59% pass_rate_2). Это всё равно **в 2× выше** чем у 30B-A3B (25.5% и 17.9% соответственно)

### Сравнение per-language с 30B-A3B

| Язык | 30B-A3B pass_2 | Coder Next pass_2 | Δ |
|------|----------------|-------------------|---|
| JavaScript | 32.7% | **80.9%** | **+48.2pp** ⭐⭐ |
| Python | 23.5% | **76.5%** | **+53.0pp** ⭐⭐ |
| C++ | 32.0% | 61.1% | +29.1pp |
| Java | 25.5% | 59.1% | +33.6pp |
| Go | 17.9% | 58.3% | +40.4pp |

80B-A3B даёт **+29-53pp везде**, наибольший gain на Python и JavaScript. Это объясняет почему Coder Next -- best balanced default: качество не просто выше, оно **значимо выше на каждом языке**.
