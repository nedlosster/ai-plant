...
# Создать venv для бенчмарков (если ещё нет)
python3 -m venv ~/.venvs/aider-bench
source ~/.venvs/aider-bench/bin/activate

# Установить Aider
pip install aider-chat aider-bench

# Проверить
aider --version  # должно показать версию 0.x.x
```

### Клонирование dataset

Aider Polyglot использует Exercism repos. Клонируются автоматически при первом запуске бенчмарка, но можно подготовить заранее:

```bash
# Aider клонирует в ~/.aider/cache/polyglot
mkdir -p ~/.aider/cache
# При первом запуске бенчмарка aider скачает dataset (~500 MB)
```
