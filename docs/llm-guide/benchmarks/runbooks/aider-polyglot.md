...
### Установка Aider и polyglot-benchmark

В aider 0.86+ benchmark вынесен из CLI -- запускается через `benchmark/benchmark.py` из исходного репо. Pypi-пакет `aider-bench` не существует. Workflow:

```bash
ssh -A -p 2277 nedlosster@79.164.89.150
cd ~/projects/ai-plant

# 1. venv для бенчмарков
python3 -m venv ~/.venvs/aider-bench

# 2. Клонировать репо aider в ~/projects/aider/
git clone https://github.com/Aider-AI/aider.git ~/projects/aider

# 3. Клонировать polyglot-benchmark dataset (внутрь aider/)
...
# 4. Установить aider в editable режиме с dev-зависимостями
~/.venvs/aider-bench/bin/pip install -e ~/projects/aider'[dev]'

# 5. Проверить
~/.venvs/aider-bench/bin/aider --version  # 0.86.x+
~/.venvs/aider-bench/bin/python ~/projects/aider/benchmark/benchmark.py --help | head -5
```

### Bypass safety-guard

`benchmark.py` требует переменную `AIDER_DOCKER=1` (safety -- запрещает запуск unvetted LLM-кода вне Docker). На изолированном inference-сервере с собственными моделями это допустимо. Скрипт `bench-aider.sh` устанавливает её автоматически.
