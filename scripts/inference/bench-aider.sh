...
#   ./bench-aider.sh --full  --model qwen3-coder-30b --port 8080
#
...
echo "Watchdog:   task-timeout=${TASK_TIMEOUT}s, total-timeout=${TOTAL_TIMEOUT}s"
echo ""
...
if [[ $EXIT_CODE -eq 0 ]]; then
    ...
    echo "=== Прерван (exit $EXIT_CODE) за $((DURATION / 3600))h $(((DURATION % 3600) / 60))m ==="
    echo "Скорее всего сработал watchdog -- проверь последние строки лога."
fi
echo "Лог: $LOG"
echo ""
